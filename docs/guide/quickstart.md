...
# Agency account
creds = Credentials.create("AGENCY_ID", "password", is_customer=False)

# Customer account
creds = Credentials.create("user@email.com", "password", is_customer=True)
```
...
async def main():
    creds = Credentials.create("YOUR_ID", "YOUR_PASSWORD", is_customer=False)
