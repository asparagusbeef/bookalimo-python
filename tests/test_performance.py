...
from bookalimo.schemas.booking import Location, LocationType, PriceRequest, PriceResponse, RateType
...
class TestPerformance:
    ...
    def test_credential_hash_performance(self):
        ...
        assert elapsed < 1.0, f"Credential creation took {elapsed:.3f}s for 100 credentials"
        
        # Average time per credential should be reasonable
        avg_time_per_cred = elapsed / 100
        assert avg_time_per_cred < 0.01, f"Average time per credential: {avg_time_per_cred:.4f}s"
    ...
    async def test_async_concurrent_requests(self, sample_locations):
        ...
        async with AsyncBookalimo(credentials=credentials) as client:
            ...
            avg_time = elapsed / 50
            assert avg_time < 0.1, f"Average time per concurrent request: {avg_time:.4f}s"
    ...
    def test_schema_validation_performance(self, sample_locations):
        ...
        assert avg_validation_time < 0.002, f"Average validation time: {avg_validation_time:.5f}s"
    ...
    def test_location_creation_performance(self):
        ...
        for i in range(1000):
            location = Location(
                type=LocationType.ADDRESS if i % 2 == 0 else LocationType.AIRPORT,
                name=f"Test Location {i}",
                line1=f"Address Line 1 {i}",
                city=f"City{i}",
                state="TS",
                country="US",
                ...
                airport_code=f"T{i:02d}" if i % 2 == 1 else None
            )
    ...
    async def test_sustained_load(self, sample_locations):
        ...
        async with AsyncBookalimo(credentials=credentials) as client:
            ...
            assert requests_per_second > 50, f"Only {requests_per_second:.1f} requests/second"
...
class TestScalability:
    ...
    def test_client_creation_overhead(self):
        ...
        assert avg_time_per_client < 0.02, f"Average time per client: {avg_time_per_client:.4f}s"
    ...
    async def test_async_client_creation_overhead(self):
        ...
        assert elapsed < 3.0, f"100 async client create/destroy cycles took {elapsed:.3f}s"
