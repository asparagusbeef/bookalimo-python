...
class TestAsyncBookalimo:
    ...
    def test_init_with_both_credentials_and_transport(self, credentials, async_transport):
    ...
    async def test_close_cleanup(self, credentials):
        ...
        client._transport.aclose = Mock(return_value=None)
        
        await client.aclose()
        client._transport.aclose.assert_called_once()

    @pytest.mark.asyncio
    async def test_close_cleanup_with_places_client(self, credentials):
        """Test cleanup with Google Places client."""
        with patch("bookalimo.client._GOOGLE_PLACES_AVAILABLE", True), \
             patch("bookalimo.client._AsyncGooglePlaces") as mock_places_class:
            
            mock_places_instance = Mock()
            mock_places_instance.aclose = Mock(return_value=None)
...
class TestBookalimo:
    ...
    def test_init_with_both_credentials_and_transport_warns(self, credentials, sync_transport):
