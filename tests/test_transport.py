...
from bookalimo.schemas.booking import PriceRequest, PriceResponse, RateType, Location, LocationType
...
class TestCredentials:
    ...
    def test_credentials_model_validation(self):
        ...
        creds = Credentials(
            id="user123",
            password_hash="a" * 64,  # Valid hex string
            is_customer=False
        )
        
        assert creds.id == "user123"
        assert creds.password_hash == "a" * 64
        assert creds.is_customer is False
...
class TestRetryLogic:
    ...
    def test_sync_retry_success_first_attempt(self):
        ...
        result = sync_retry(mock_func, max_retries=3, backoff=0.1)
    ...
    def test_sync_retry_exhausts_retries(self):
        ...
        with patch("time.sleep"):
            with pytest.raises(httpx.ConnectError):
                sync_retry(mock_func, max_retries=2, backoff=0.1)
    ...
    def test_sync_retry_non_retriable_exception(self):
        ...
        with pytest.raises(ValueError):
            sync_retry(mock_func, max_retries=3, backoff=0.1)
    ...
    async def test_async_retry_success_first_attempt(self):
        ...
        result = await async_retry(mock_func, max_retries=3, backoff=0.1)
    ...
    async def test_async_retry_exhausts_retries(self):
        ...
        with patch("asyncio.sleep"):
            with pytest.raises(httpx.ConnectError):
                await async_retry(mock_func, max_retries=2, backoff=0.1)
...
class TestAsyncTransport:
    ...
    async def test_init_default_client(self, credentials):
        ...
        assert transport.base_url == "https://sandbox.bookalimo.com"
        assert transport.credentials == credentials
        assert transport.retries == 3  # Default
        assert transport.backoff == 1.0  # Default
    ...
    @pytest.mark.asyncio
    async def test_post_success(self, transport, sample_pickup_location, sample_dropoff_location):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            ...
            result = await transport.post("/booking/price/", request_model, PriceResponse)
    ...
    @pytest.mark.asyncio
    async def test_post_with_credentials_injection(self, transport, sample_pickup_location, sample_dropoff_location):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            ...
            assert request_data["credentials"]["password_hash"] == transport.credentials.password_hash

    @pytest.mark.asyncio
    async def test_post_http_error(self, transport, sample_pickup_location, sample_dropoff_location):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            respx_mock.post("/booking/price/").mock(
                return_value=httpx.Response(
                    400, 
                    json={"error": "Bad Request", "code": "INVALID_REQUEST"}
                )
            )
    ...
    @pytest.mark.asyncio
    async def test_post_timeout_error(self, transport, sample_pickup_location, sample_dropoff_location):
    ...
    @pytest.mark.asyncio
    async def test_post_connection_error(self, transport, sample_pickup_location, sample_dropoff_location):
    ...
    @pytest.mark.asyncio
    async def test_post_without_credentials(self, sample_pickup_location, sample_dropoff_location):
    ...
    @pytest.mark.asyncio
    async def test_request_logging(self, transport, sample_pickup_location, sample_dropoff_location, caplog):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            ...
            request_logs = [msg for msg in log_messages if "POST /booking/price/" in msg]
...
class TestSyncTransport:
    ...
    def test_post_success(self, transport, sample_pickup_location, sample_dropoff_location):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            ...
            assert isinstance(result, PriceResponse)
            assert result.token == "sync-test-token-123"
            assert result.total == 150.00

    def test_post_http_error(self, transport, sample_pickup_location, sample_dropoff_location):
        ...
        with respx.mock(base_url="https://api.test.com") as respx_mock:
            ...
            with pytest.raises(BookalimoHTTPError) as exc_info:
                transport.post("/booking/price/", request_model, PriceResponse)
...
class TestTransportIntegration:
    ...
    def test_sync_transport_with_retry_on_timeout(self, credentials):
        ...
        success_response = {"token": "timeout-recovery", "total": 100, "currency": "USD"}
